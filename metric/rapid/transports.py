...
from metric.geostrophy \
   import calc_dh, calc_vgeo, calc_vgeo_td, calc_vgeo_endpoint, \
          ...
          calc_ek, rapid_mass_balance
from metric.rapid import output


class RAPID_Transports(object):
...
def calc_transports_from_sections(config, v, tau, t_on_v, s_on_v, ssh_on_v):
    ...
    vgeo, mbal = rapid_mass_balance(vgeo, ek, fc_minlon, wbw_maxlon, int_maxlon)
    ...
    fc_trans = RAPID_Transports(vgeo, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, fcmin, fcmax, sref=sref)
    # Western-boundary wedge transports
    wbw_trans = RAPID_Transports(vgeo, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, wbwmin, wbwmax, sref=sref)
    # Gyre interior transports
    int_trans = RAPID_Transports(vgeo, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, intmin, intmax, sref=sref)
    # Gyre interior transports using model velocities
    int_trans_mod = RAPID_Transports(v, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, intmin, intmax, sref=sref)
    # Ekman transports
    ek_trans = RAPID_Transports(ek, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, intmin, intmax, sref=sref)
    # Total section transports using RAPID approximation
    rapid_trans = RAPID_Transports(vrapid, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, fcmin, intmax, sref=sref)
    # Total section transports using model velocities
    model_trans = RAPID_Transports(v, t_on_v, s_on_v, ssh_on_v, dh.bounds_data, rho.bounds_data, fcmin, intmax, sref=sref)

    # Create netcdf object for output/plotting
    trans = output.create_netcdf(config, rapid_trans, model_trans, fc_trans,
                                 wbw_trans, int_trans, int_trans_mod, ek_trans, mbal)
